def kafkaSendFiles(directories, files, basicmetadata, df_metadata, kafkaProducer, kafkaTopic):
    ...
    # Write to Blob Storage
        ...
        if 'lat' in df_metadata:newEntry["lat"]=df_metadata['lat'][j]
        if 'lon' in df_metadata:newEntry["lon"]=df_metadata['lon'][j]
        if 'alt' in df_metadata:newEntry["alt"]=df_metadata['alt'][j]
        if 'vf' in df_metadata:newEntry["velocity_lon"]=df_metadata['vf'][j]
        if 'vl' in df_metadata:newEntry["velocity_lat"]=df_metadata['vl'][j]
        if 'vu' in df_metadata:newEntry["velocity_alt"]=df_metadata['vu'][j]
